...
local function freecamFrame ()
	...
	local speedc = math.max((2^(lSecession.variables['Speed'][2]-1))/200,0.001)
	
    local mspeed = speedc or 20
    ...
    setCameraMatrix ( camPosX, camPosY, camPosZ, camTargetX, camTargetY, camTargetZ,0,lSecession.variables['Field Of View'][1] )
end
...
local function freecamMouse (cX,cY,aX,aY)
	...
	local sensativity = (lSecession.variables['Sensativity'][2])*0.014
	
    rotX = rotX + aX * (sensativity / (getKeyState('lalt') and 5 or 1)) * 0.01745
    rotY = rotY - aY * (sensativity / (getKeyState('lalt') and 5 or 1)) * 0.01745
...
function setFreecamEnabled (x, y, z,xa,ya,za)
	if isFreecamEnabled() then
		return false
	end

	if (x and y and z) then
	    setCameraMatrix ( x, y, z, xa, ya, za,0,lSecession.variables['Field Of View'][1] )
	end
